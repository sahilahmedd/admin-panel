...
interface CreateEventData {
  ...
  EVNT_UPTO_DT: string;
  EVET_ACTIVE_YN: string;
  ENVT_CATE_ID: number | string;
  ENVT_CATE_CATE_ID?: number | string;
...
const AddEventPage = () => {
  ...
  const [eventDataHi, setEventDataHi] = useState({
    ENVT_DESC: "",
    ENVT_EXCERPT: "",
    ENVT_DETAIL: "",
    ENVT_CITY: "",
  ...
  useEffect(() => {
    ...
    fetchCategories();
    if (eventData.ENVT_GALLERY_IMAGES.length > 0) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const resultEn = await eventService.createEvent(payloadEn);
      if (!resultEn) throw new Error("English event creation failed");
      ...
      const resultHi = await eventService.createEvent(payloadHi);
      if (!resultHi) throw new Error("Hindi event creation failed");
      toast.success("Event created successfully in both languages!");
      router.push("/events");
    } catch (error) {
  ...
  return (
    <div className="p-5 max-w-6xl mx-auto bg-white shadow-lg rounded-lg mt-10">
      ...
      <p className="text-sm text-gray-600 mb-6 text-center">
        Fill in the event details and translations. Use the tabs to switch
        between languages. All fields marked with * are required.
      </p>
              ...
              {editorLoaded && (
                <>
                  <ClientSideCustomEditor
                    ...
                    onChange={(html) =>
                      setEventData((prev) => ({ ...prev, ENVT_DETAIL: html }))
                    }
                  />
                  {errors.ENVT_DETAIL && (
                    <p className="text-red-500 text-sm mt-1">
