// File: app/admin/pages/page.tsx (for App Router)
...
import { Page, ApiResponse } from "@/utils/types"; // Adjust path as per your types file location
...
function PageList() {
  ...
  useEffect(() => {
    const fetchPages = async () => {
      try {
        ...
        // Use 'any' or more specific error type if known
  ...
  // Type the 'id' parameter as number
  const handleDelete = async (id: number) => {
    if (
      !window.confirm(`Are you sure you want to delete page with ID: ${id}?`)
    ) {
      return;
    }
    try {
      ...
      const data: ApiResponse<any> = await response.json(); // Data might be empty or a message
